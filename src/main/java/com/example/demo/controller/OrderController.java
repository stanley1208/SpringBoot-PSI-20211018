...
public class OrderController {
	...
	public String update(Order order, @PathVariable("oid") Long oid) {
		order.setId(oid);// 重要!不然會變成新增
		orderRepository.save(order);
		return "redirect:/order/";
	}

	@RequestMapping("/delete/{oid}")
	public String delete(Order order, @PathVariable("oid") Long oid) {
		order.setId(oid);
	...
	//BindingResult result,Model model注意宣告順序
	public String addItem(OrderItem orderItem,BindingResult result,Model model,@PathVariable("oid") Long oid) {
		//驗證資料
